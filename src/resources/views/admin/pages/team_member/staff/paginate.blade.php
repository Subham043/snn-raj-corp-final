...
<div class="page-content">
    <div class="container-fluid">
        ...
        <!-- start page title -->
        @include('admin.includes.breadcrumb', ['page'=>'Staffs', 'page_link'=>route('team_member.staff.paginate.get'), 'list'=>['List']])
        <!-- end page title -->

        <div class="row" id="image-container">
            <div class="col-lg-12">
                <form id="countryForm" method="post" action="{{route('team_member.staff.heading.post')}}" enctype="multipart/form-data">
                @csrf
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">Staff Heading</h4>
                        </div><!-- end card header -->
                        <div class="card-body">
                        ...
                        <div id="customerList">
                            <div class="row g-4 mb-3">
                                <div class="col-sm-auto">
                                    <div>
                                        @can('create team member staffs')
                                        <a href="{{route('team_member.staff.create.get')}}" type="button" class="btn btn-success add-btn" id="create-btn"><i class="ri-add-line align-bottom me-1"></i> Create</a>
                                        @endcan
                                    </div>
                                </div>
                                <div class="col-sm">
                                    @include('admin.includes.search_list', ['link'=>route('team_member.staff.paginate.get'), 'search'=>$search])
                                </div>
                            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex gap-2">
                                                    ...
                                                    <div class="edit">
                                                        <a href="{{route('team_member.staff.update.get', $item->id)}}" class="btn btn-sm btn-primary edit-item-btn">Edit</a>
                                                    </div>
                                                    @endcan

                                                    @can('delete team member staffs')
                                                    <div class="remove">
                                                        <button class="btn btn-sm btn-danger remove-item-btn" data-link="{{route('team_member.staff.delete.get', $item->id)}}">Delete</button>
                                                    </div>
                                                    @endcan
                                                </div>
                                            </td>
...
.addField('#sub_heading', [
  ...
  .onSuccess(async (event) => {
    ...
    try {
        ...
        const response = await axios.post('{{route('team_member.staff.heading.post')}}', formData)
