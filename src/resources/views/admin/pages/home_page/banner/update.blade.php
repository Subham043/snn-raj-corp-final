								...
								<div class="container-fluid">
												...
												@can("list home page banners")
																@include("admin.includes.breadcrumb", [
																				"page" => "Banner",
																				"page_link" => route("home_page.banner.paginate.get"),
																				"list" => ["Update"],
																])
												@endcan
												<!-- end page title -->
												...
												<div class="row" id="image-container">
																...
																<div class="col-lg-12">
																				<form id="countryForm" method="post" action="{{ route("home_page.banner.update.post", $data->id) }}"
																																...
																																<div class="live-preview">
																																				<div class="row gy-4" id="image_row">
																																								<div class="col-xxl-6 col-md-6">
																																												...
																																												<p>
																																																<code>Note: </code> Banner Size : 1415 x 750
																																												</p>
																																												@if (!empty($data->banner_image_link))
																																																<img src="{{ $data->banner_image_link }}" alt=""
																																																				class="img-preview">
																																												@endif
																																								</div>
																																								<div class="col-xxl-6 col-md-6">
																																												@include("admin.includes.file_input", [
																																																"key" => "banner_mobile_image",
																																																"label" => "Mobile Image",
																																												])
																																												<p>
																																												...
																																												@if (!empty($data->banner_mobile_image_link))
																																																<img src="{{ $data->banner_mobile_image_link }}" alt=""
																																																				class="img-preview">
																																												@endif
																																								</div>
																																								...
																																								<div class="col-xxl-4 col-md-4">
																																												@include("admin.includes.input", [
																																																...
																																																"label" => "Image Alt",
																																																"value" => $data->banner_image_alt,
																																												])
																																								</div>
																																								<div class="col-xxl-4 col-md-4">
																																												@include("admin.includes.input", [
																																																"key" => "banner_image_title",
																																																"label" => "Image Title",
																																																"value" => $data->banner_image_title,
																																												])
																																								</div>
								...
								validation
												...
												.onSuccess(async (event) => {
																...
																try {
																				...
																				const response = await axios.post('{{ route("home_page.banner.update.post", $data->id) }}',
																								formData)
																				...
																				if (error?.response?.data?.errors?.button_link) {
																								validation.showErrors({
																												'#button_link': error?.response?.data?.errors?.button_link[0]
																								})
																				}
																				if (error?.response?.data?.errors?.banner_image_title) {
																								validation.showErrors({
																												'#banner_image_title': error?.response?.data?.errors?.banner_image_title[0]
																								})
																				}
																				if (error?.response?.data?.errors?.banner_image_alt) {
																								validation.showErrors({
																												'#banner_image_alt': error?.response?.data?.errors?.banner_image_alt[0]
																								})
																				}
																				if (error?.response?.data?.errors?.banner_image) {
																								validation.showErrors({
																												'#banner_image': error?.response?.data?.errors?.banner_image[0]
																								})
																				}
																				if (error?.response?.data?.errors?.banner_mobile_image) {
																								validation.showErrors({
																												'#banner_mobile_image': error?.response?.data?.errors?.banner_mobile_image[0]
																								})
																				}
