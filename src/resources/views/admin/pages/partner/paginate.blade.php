...
<div class="page-content">
    <div class="container-fluid">
        ...
        <div class="row" id="image-container">
            <div class="col-lg-12">
                <form id="countryForm" method="post" action="{{route('partner.heading.post')}}" enctype="multipart/form-data">
                @csrf
                    <div class="card">
                            ...
                            <div class="live-preview">
                                <div class="row gy-4">
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'heading', 'label'=>'Heading', 'value'=>!empty($partnerHeading) ? (old('heading') ? old('heading') : $partnerHeading->heading) : old('heading')])
                                        <p>
                                            <code>Note: </code> Put the text in between span tags to make it highlighted
                                        </p>
                                    </div>
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'sub_heading', 'label'=>'Sub Heading', 'value'=>!empty($partnerHeading) ? (old('sub_heading') ? old('sub_heading') : $partnerHeading->sub_heading) : old('sub_heading')])
                                    </div>
...
.addField('#sub_heading', [
  ...
  .onSuccess(async (event) => {
    ...
    try {
        ...
        const response = await axios.post('{{route('partner.heading.post')}}', formData)
