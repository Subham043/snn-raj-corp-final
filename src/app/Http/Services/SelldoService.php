...
class SelldoService
{
    ...
    {
        try {
            $response = Http::asForm()->post('https://app.sell.do/api/leads/create?sell_do[form][lead][name]='.$name.'&sell_do[form][lead][email]='.$email.'&sell_do[form][lead][phone]='.$phone.'&sell_do[form][lead][source]='.$source.'&sell_do[campaign][srd]=64a7bd5c0ad1ff18a297393b&sell_do[form][lead][project_id]='.$project.'&api_key=26c5b0ac69821ba28d6293355d641ec9');
            $data = json_decode($response->body());
            return true;
        } catch (\Throwable $th) {
            return false;
    ...
    {
        try {
            $response = Http::asForm()->post('https://app.sell.do/api/leads/create?sell_do[form][lead][name]='.$name.'&sell_do[form][lead][email]='.$email.'&sell_do[form][lead][phone]='.$phone.'&sell_do[form][lead][source]='.$source.'&sell_do[campaign][srd]=64a7c07b0ad1ff19f1e846f4&sell_do[form][lead][project_id]='.$project.'&sell_do[form][lead][sales]='.$executive_name.'&api_key=26c5b0ac69821ba28d6293355d641ec9');
            $data = json_decode($response->body());
            return true;
        } catch (\Throwable $th) {
            return false;
